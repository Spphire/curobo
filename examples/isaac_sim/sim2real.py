...
def main():
    ...
    while simulation_app.is_running():
        ...
        try:
            if len(list(json.loads(q.content).values()))==6:
                pass
        except:
            quit()

        cu_js = JointState(
            position=tensor_args.to_device(np.array(list(json.loads(q.content).values()))),
        ...
        temp_q = cmd_state.position.cpu().numpy()
        m=dict()
        m['q']=temp_q.flatten().tolist()
        requests.post("http://10.53.21.95:8000/move",json.dumps(m))
