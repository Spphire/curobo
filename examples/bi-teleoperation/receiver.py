...
class Receiver(Thread):
    ...
    def receive(self):
        try:
            ...
            # 夹爪部分
            self.controller.left_robot.gripper.stop()
            self.controller.left_robot.gripper.move(0.1-s["leftHand"]["squeeze"]/9, 1, 20)
            self.controller.right_robot.gripper.stop()
            self.controller.right_robot.gripper.move(0.1-s["rightHand"]["squeeze"]/9, 
                                                     1, # API中显示似乎是速度但修改没有效果
                                                     20)
