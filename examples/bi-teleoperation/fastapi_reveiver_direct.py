...
bi.left_robot.robot.setMode(bi.left_robot.mode.NRT_CARTESIAN_MOTION_FORCE)
...
def MainThread():
    while True:
        ...
        if mes.rightHand.cmd==3 or mes.leftHand.cmd==3:
            bi.left_robot.robot.setMode(bi.left_robot.mode.NRT_JOINT_POSITION)
            bi.right_robot.robot.setMode(bi.right_robot.mode.NRT_JOINT_POSITION)
            bi.birobot_go_home()
            bi.left_robot.robot.setMode(bi.left_robot.mode.NRT_CARTESIAN_MOTION_FORCE)
            bi.right_robot.robot.setMode(bi.right_robot.mode.NRT_CARTESIAN_MOTION_FORCE)
            continue
        ...
        if bi.left_robot.homing_state:
            ...
            if mes.leftHand.cmd==2:
                if bi.left_robot.tracking_state:
                    ...
                    bi.left_robot.set_start_tcp_raw(l_pos_from_unity)
        ...
        if bi.right_robot.homing_state:
            ...
            if mes.rightHand.cmd==2:
                if bi.right_robot.tracking_state:
                    ...
                    bi.right_robot.set_start_tcp_raw(r_pos_from_unity)
        ...
        if not bi.left_robot.homing_state and not bi.right_robot.homing_state:
            ...
            if np.linalg.norm(left_target[:3]-bi.left_robot.get_current_tcp_raw()[:3])>threshold:
                if bi.left_robot.tracking_state:
                    print("left robot lost sync")
                bi.left_robot.tracking_state=False
            if bi.left_robot.tracking_state:
                bi.left_robot.tcp_move(left_target)

            right_target = bi.right_robot.get_relative_target(r_pos_from_unity)
            if np.linalg.norm(right_target[:3]-bi.right_robot.get_current_tcp_raw()[:3])>threshold:
                if bi.right_robot.tracking_state:
                    print("right robot lost sync")
                bi.right_robot.tracking_state=False
            if bi.right_robot.tracking_state:
                bi.right_robot.tcp_move(right_target)
